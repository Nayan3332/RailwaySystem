...
public class AddTrainFrame extends JFrame {
	...
	public AddTrainFrame(AdminDashboard parent, String adminName) {
		this.parent = parent;
		setTitle("Admin - Register New Train");
		...
		JLabel lblTitle = new JLabel("TRAIN REGISTRATION");
		...
		String[] labels = { "Train Number:", "Train Name:", "Source:", "Destination:", "Ticket Price:",
				"Total Seats:" };
		JTextField[] fields = { txtNo = new JTextField(), txtName = new JTextField(), txtSrc = new JTextField(),
				txtDest = new JTextField(), txtPrice = new JTextField(), txtSeats = new JTextField() };
		...
		JButton btnSave = new JButton("CREATE TRAIN");
	...
	private void saveTrainLogic() {
		try {
			...
			Train t = new Train(Integer.parseInt(txtNo.getText().trim()), txtName.getText().trim(),
					txtSrc.getText().trim(), txtDest.getText().trim(), Double.parseDouble(txtPrice.getText().trim()),
					Integer.parseInt(txtSeats.getText().trim()));

			Train_DAO dao = new Train_DAO();
			if (dao.addTrain(t)) {
				// Scheduled
				dao.updateTrainTracking(t.getTrainNo(), t.getSource(), "Scheduled");

				JOptionPane.showMessageDialog(this, "Train added and Tracking initialized!");
				parent.loadTrains();
				dispose();
			} else {
